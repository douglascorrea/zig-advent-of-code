...
pub fn main() !void {
    ...
    const food_slice = list_of_elf_food.items;
    std.mem.sort(i32, food_slice, {}, std.sort.desc(i32));
    std.debug.print("\nThe first elf with most food has: {}", .{food_slice[0]});
    std.debug.print("\nThe second elf with most food has: {}", .{food_slice[1]});
    std.debug.print("\nThe third elf with most food has: {}", .{food_slice[2]});
    var total_sum = food_slice[0] + food_slice[1] + food_slice[2];
    std.debug.print("\nThey are carrying a total of : {}", .{total_sum});
}
